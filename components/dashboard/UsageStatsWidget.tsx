...
export function UsageStatsWidget({ compact = false }: UsageStatsWidgetProps) {
  ...
  // Translations
  const t = {
    title: lang === 'es' ? 'Uso del Plan' : 'Plan Usage',
    subtitle: lang === 'es' ? 'Estadísticas de este mes' : 'This month\'s statistics',
    upgrade: lang === 'es' ? 'Mejorar Plan' : 'Upgrade Plan',
    unlimited: lang === 'es' ? 'Ilimitado' : 'Unlimited',
    notAvailable: lang === 'es' ? 'No disponible' : 'Not available',
    used: lang === 'es' ? 'usado' : 'used',
    remaining: lang === 'es' ? 'restantes' : 'remaining',
    features: {
      ats_export: {
        name: lang === 'es' ? 'Exportaciones ATS' : 'ATS Exports',
        description: lang === 'es' ? 'PDF/DOCX este mes' : 'PDF/DOCX this month',
      },
      ai_request: {
        name: lang === 'es' ? 'Solicitudes IA' : 'AI Requests',
        description: lang === 'es' ? 'Mejoras de texto' : 'Text improvements',
      },
      stamp_request: {
        name: lang === 'es' ? 'Verificaciones' : 'Verifications',
        description: lang === 'es' ? 'Stamps solicitados' : 'Stamps requested',
      },
    },
    periodInfo: lang === 'es' ? 'Se reinicia cada mes' : 'Resets monthly',
  };
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className={`font-medium ${isNotAvailable ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-gray-100'}`}>
                      {isNotAvailable ? (
                        <span className="text-gray-400 dark:text-gray-500">{t.notAvailable}</span>
                      ) : isUnlimited ? (
                        <span className="text-green-600 dark:text-green-400">{t.unlimited}</span>
                      ) : (
                ...
                {isNotAvailable ? (
                  <div className="text-center py-2">
                    <div className="text-sm text-gray-400 dark:text-gray-500 font-medium">
                      {lang === 'es' ? 'No incluido en tu plan' : 'Not included in your plan'}
                    </div>
                  </div>
                ) : isUnlimited ? (
