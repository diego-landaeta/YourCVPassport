...
const ShareModal: React.FC<ShareModalProps> = ({
  ...
  const handlePlatformClick = (platform: typeof PLATFORMS[number]) => {
    const url = platform.getUrl(postUrl, shareText, authorName);
  ...
  return (
    ...
    >
      ...
      <div
        ref={modalRef}
        className="relative w-full sm:max-w-sm bg-white dark:bg-dark-bg-secondary rounded-t-2xl sm:rounded-2xl shadow-2xl border border-gray-200 dark:border-dark-border overflow-hidden animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-full text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-dark-bg-tertiary transition-colors"
          >
        ...
        <div className="px-5 py-4">
          <div className="flex items-center gap-2.5 bg-gray-50 dark:bg-dark-bg-tertiary rounded-xl px-3.5 py-2.5 border border-gray-200 dark:border-dark-border">
            ...
            <input
              ...
              readOnly
              value={postUrl}
              className="flex-1 text-xs text-gray-600 dark:text-gray-400 bg-transparent truncate focus:outline-none select-all min-w-0"
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex-shrink-0 ${
                copied
                  ...
                  : 'bg-cv-blue text-white hover:bg-blue-700 active:scale-95'
              }`}
            >
              {copied ? (
                <>
